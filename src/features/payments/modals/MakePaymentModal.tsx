...
import { parseBatchPaymentCSV, downloadBatchPaymentCSV, generateBatchPaymentCSV, generateCSVPreview } from '../../../utils/batchPaymentUtils';
...
export function MakePaymentModal({
  ...
  // Normalize wallets data early - handle both array and API response object
  ...
  const [currentRecipient, setCurrentRecipient] = useState<BatchCategoryItem>({
    recipientName: '',
    amount: 0,
    description: '',
  ...
  const [uploadedRecipients, setUploadedRecipients] = useState<BatchCategoryItem[]>([]);
  ...
  useEffect(() => {
    if (selectedApproval && isOpen) {
      ...
      if (selectedApproval.paymentRequestType === 'bulk' && selectedApproval.batchPaymentDetails) {
  ...
  useEffect(() => {
    if (selectedExpense && isOpen) {
      ...
      if (selectedExpense.paymentRequestType === 'bulk' && selectedExpense.batchPaymentDetails) {
        setIsBatchPayment(true);
  ...
  const getSourceItems = () => {
    switch (paymentSource) {
      case 'expense':
        // Show all approved expenses that haven't been paid yet
        // Create a Set of expense IDs that have already been paid
        const paidExpenseIds = new Set(payments.map(p => p.expenseId).filter(Boolean));

        return expenses
          .filter(e => e.status === 'Approved' && !paidExpenseIds.has(e.id))
          .map(e => ({
            value: e.id,
  ...
  useEffect(() => {
    ...
    const loadFromSource = () => {
      if (paymentSource === 'expense') {
        ...
        if (exp) {
          // Handle new batch expense format
          if (exp.expenseRequestType === 'batch' && exp.batchCategories) {
            setIsBatchPayment(true);
          ...
          // Handle legacy bulk payment format
          else if (exp.paymentRequestType === 'bulk' && exp.batchPaymentDetails && exp.batchPaymentDetails.length > 0) {
            setIsBatchPayment(true);
            const recipients = exp.batchPaymentDetails.map((r: any) => ({
              name: r.name,
              idNumber: r.idNumber,
              phone: r.phone,
            ...
            // clear recipients when switching to an expense without batch details
            setBatchRecipients([]);
          }
        }
        ...
        if (req && req.batchPaymentDetails && req.batchPaymentDetails.length > 0) {
          setIsBatchPayment(true);
          const recipients = req.batchPaymentDetails.map((r: any) => ({
            name: r.name,
            idNumber: r.idNumber,
            phone: r.phone,
            paybillNumber: r.paybillNumber,
        ...
        if (app && app.batchPaymentDetails && app.batchPaymentDetails.length > 0) {
          setIsBatchPayment(true);
          const recipients = app.batchPaymentDetails.map((r: any) => ({
            name: r.name,
            idNumber: r.idNumber,
            phone: r.phone,
            paybillNumber: r.paybillNumber,
  ...
  // (Template download removed — downloads should come from expense-created CSV)
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={supplier ? `Pay Supplier: ${supplier.name}` : "Make Payment Outside Platform"}
      size="lg"
    >
      <div className="space-y-6">
        ...
        {isBatchPayment ? (
          ...
          <div className="space-y-4">
                  ...
                  <div className="mt-4">
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <div className="flex justify-between font-semibold">
                        ...
                        <span>{csvPreviewData.length} rows</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {/* Wallet Balance Display - For Batch Payment */}
            {batchRecipients.length > 0 && (() => {
              // Everyone (including admin) sees their personal wallet
              const userWallet = walletsArray.find(w => w.type === 'USER' && w.name === `${currentUser?.firstName} ${currentUser?.lastName}`);
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      {totalBatchAmount > walletBalance && (
        ...
        {/* Wallet Balance Display - Only for Single Payment */}
        {!isBatchPayment && (() => {
          // Everyone (including admin) sees their personal wallet
          const userWallet = walletsArray.find(w => w.type === 'USER' && w.name === `${currentUser?.firstName} ${currentUser?.lastName}`);
          const walletBalance = userWallet?.balance || 0;
