...
export function EditEvent({
  ...
  const [formData, setFormData] = useState({
    type: 'Event' as 'Event' | 'Activation' | 'Operation',
    name: '',
    category: '',
    client: '',
    brand: '',
  ...
  useEffect(() => {
    if (event) {
      setFormData({
        type: (event.type as 'Event' | 'Activation' | 'Operation') ?? 'Event',
        name: event.name || '',
        ...
        projectLead: typeof event.projectLead === 'string' ? event.projectLead : '',
        location: event.location || '',
        product: event.product || '',
        campaignName: event.campaignName || '',
        activationChannel: event.activationChannel || '',
      });
    }
  }, [event]);
  ...
  const categories = formData.type === 'Event'
    ? (eventCategories || [])
  ...
  const showBudget = formData.type !== 'Operation';
  const showClient = formData.type !== 'Operation';
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold text-dark-gray">Edit {formData.type}</h1>
          </div>
