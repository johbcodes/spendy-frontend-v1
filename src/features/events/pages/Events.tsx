...
export function Events({ onNavigate, onOpenModal, onArchiveEvent, events, payments, currentUser }: EventsProps) {
  ...
  const handleExport = (format: 'csv' | 'pdf' | 'excel') => {
    ...
    if (format === 'csv') exportToCSV(data, 'events');
    else if (format === 'pdf') exportToPDF(data, 'events', 'Events Report');
    else exportToExcel(data, 'events');
  };

  const columns: Column<Event>[] = [
    { key: 'name', label: 'Event Name', sortable: true, mobilePriority: 1 },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-dark-gray">Events</h1>
          <p className="text-gray-600 mt-1">Manage all your events, activations, and operations</p>
        </div>
        {canModify && (
          <Button onClick={() => onOpenModal('new-event')}>
      ...
      <Card className="p-4">
        ...
        {showFilters && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-4 mt-4 border-t border-gray-200">
            ...
            <Select label="Type" value={selectedType} onChange={e => setSelectedType(e.target.value)}
              options={[{ value: 'all', label: 'All Types' }, { value: 'event', label: 'Project' }, { value: 'activation', label: 'Activation' }, { value: 'operation', label: 'Operation' }]} />
      ...
      <div className="space-y-8">
        {(['Project', 'Activation', 'Operation'] as const).map(typeKey => {
          const items = filteredEvents.filter(e => e.status === 'Active' && e.type === typeKey);
          const title = typeKey === 'Project' ? 'Projects' : typeKey === 'Activation' ? 'Activations' : 'Operations';
          return (
            <div key={typeKey}>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-dark-gray">{title} ({items.length})</h3>
              </div>
              {items.length === 0
                ? <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-600">No active {title.toLowerCase()}.</div>
                : (
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-dark-gray">All Events</h3>
          <ExportButton onExport={handleExport} />
        </div>
